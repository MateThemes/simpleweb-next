...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check honeypot
    if (body._honeypot) {
      await logError(ip, 'Honeypot detected', body)
      return NextResponse.json(
        { error: 'Spam detected' },
        { status: 400 }
      )
    ...
    if (
      ...
      await logError(ip, 'Spam detected', body)
      return NextResponse.json(
        { error: 'Spam detected' },
        { status: 400 }
      )
    ...
    if (!emailRegex.test(body.email)) {
      await logError(ip, 'Invalid email format', body)
      return NextResponse.json(
    ...
    try {
      ...
      if (!addresses || addresses.length === 0) {
        await logError(ip, 'Invalid email domain', body)
        return NextResponse.json(
          { error: 'Ungültige E-Mail-Adresse.' },
          { status: 400 }
        )
