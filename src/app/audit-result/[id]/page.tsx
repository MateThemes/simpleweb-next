...
export async function generateMetadata({ params }: AuditResultPageProps): Promise<Metadata> {
  ...
  const audit = auditData as { hostname?: string };
  const hostname = audit.hostname || 'unbekannte Website';
  
  return {
    title: `SEO-Audit für ${hostname} | SimpleWebDesign`,
    description: `Detaillierter SEO-Audit Bericht für ${hostname} mit Empfehlungen und Verbesserungsvorschlägen.`,
...
export default async function AuditResultPage({ params }: AuditResultPageProps) {
  ...
  const audit = auditData as {
    ...
    onPage?: {
      title: { ok: boolean; text: string | null };
      metaDescription: { ok: boolean; text: string | null; hint?: string };
      h1: { count: number; hint?: string };
      imagesWithoutAlt: number;
      canonical: { ok: boolean; url: string | null };
  ...
  const { summary, onPage, crawlability, recommendations, hostname, timestamp } = audit;
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Analysiert am {timestamp ? new Date(timestamp).toLocaleDateString('de-DE') : 'unbekannt'}
          </p>
        </div>
              ...
              <div className="space-y-4">
                <div className="bg-white dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700">
                  ...
                  <div className={`inline-flex items-center px-2 py-1 rounded-full text-sm ${
                    ...
                    {onPage.h1?.count || 0} H1-Tag{(onPage.h1?.count || 0) !== 1 ? 's' : ''}
                  </div>
                  {onPage.h1?.hint && (
                    <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
                      {onPage.h1.hint}
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700">
                  ...
                  <div className={`inline-flex items-center px-2 py-1 rounded-full text-sm ${
                    (onPage.imagesWithoutAlt || 0) === 0 
                      ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' 
                      : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200'
                  }`}>
                    {onPage.imagesWithoutAlt || 0} Bild{(onPage.imagesWithoutAlt || 0) !== 1 ? 'er' : ''}
                  </div>
                </div>
              </div>
            </div>
